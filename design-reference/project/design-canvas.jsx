...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  const apply = React.useCallback(() => {
    ...
    if (!el) return;
    // Pan clamp: keep the artboards from being flung into the empty grid.
    // Bound the world translate so at least keepX/keepY screen-pixels of the
    // content bounding box stay inside the viewport on every axis. Measured
    // live (post-reflow) so it stays correct at every zoom level.
    if (vp) {
      const t = tf.current;
      const secs = el.querySelectorAll('[data-dc-section]');
      if (secs.length) {
        let bx0 = Infinity, by0 = Infinity, bx1 = -Infinity, by1 = -Infinity;
        secs.forEach((s) => {
          const l = s.offsetLeft, tp = s.offsetTop;
          if (l < bx0) bx0 = l; if (tp < by0) by0 = tp;
          if (l + s.offsetWidth > bx1) bx1 = l + s.offsetWidth;
          if (tp + s.offsetHeight > by1) by1 = tp + s.offsetHeight;
        });
        const vw = vp.clientWidth, vh = vp.clientHeight, s = t.scale;
        const keepX = Math.min(vw * 0.5, (bx1 - bx0) * s * 0.85);
        const keepY = Math.min(vh * 0.5, (by1 - by0) * s * 0.85);
        const loX = keepX - bx1 * s, hiX = vw - keepX - bx0 * s;
        const loY = keepY - by1 * s, hiY = vh - keepY - by0 * s;
        t.x = Math.min(Math.max(loX, hiX), Math.max(Math.min(loX, hiX), t.x));
        t.y = Math.min(Math.max(loY, hiY), Math.max(Math.min(loY, hiY), t.y));
      }
    }
  ...
  React.useEffect(() => {
    ...
    const onWheel = (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && !isMouseWheel(e)) {
        ...
        // notched mouse wheel — fixed-ratio step per click (gentle, so a
        // twitchy wheel doesn't blow the zoom across several stops at once)
        zoomAt(e.clientX, e.clientY, Math.exp(-Math.sign(e.deltaY) * 0.1));
      } else {
