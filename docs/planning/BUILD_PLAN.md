...
**STATUS: APPROVED → IN EXECUTION** (rev 5; Architect + Critic APPROVED; §14 decisions locked). **M0 COMPLETE** (2026-06-07): scaffold + i18n/hreflang/canonical/sitemap **go-no-go gate PASSED**, per-locale fonts self-hosted, import aliases everywhere, RU-only launch state, `astro check` 0 errors, deps pinned + lockfile. **M1 ✓** design-system parity (~28 components, zero-JS, aliases, fidelity-verified). **M2 ✓** core RU pages + Content Collections (21 pages). **M3 ✓** Svelte quiz island + config-driven scoring (5 vitest tests; archetype mapping/copy = clinician-confirmed YMYL placeholders). **M4 ✓** JSON-LD per page + robots.txt (AI bots) + llms.txt + Plausible (env-gated). **M5 ✓** EN locale live → **RU+EN bilingual, 40 pages**, reciprocal hreflang, root/x-default→/en/. All on branch `feat/website`, one commit per phase; `astro check` 0/0/0, build green. **Remaining (content/ops, not build):** real photos/covers/bio/copy + book names 01–04; UK+ES localization (identical pattern, content-gated); forms backend (currently stubs); clinician-confirmed quiz mapping + 3 archetype copies; verified UK/RU crisis resources; real domain via `SITE_URL` + `PUBLIC_PLAUSIBLE_DOMAIN` + deploy to Cloudflare Pages; mobile-fidelity polish pass.
